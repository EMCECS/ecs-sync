...
public class ArchiveFileSource extends FilesystemSource {
    private static final Logger log = LoggerFactory.getLogger(ArchiveFileSource.class);
    ...
    public void parseCustomOptions(CommandLine line) {
        ...
        try {
            rootFile = new TFile(new URI(sourceUri.substring(SOURCE_PREFIX.length())));
    ...
    public Iterator<FileSyncObject> iterator() {
        return Collections.singletonList(
                (FileSyncObject) new TFileSyncObject(this, mimeMap, rootFile, getRelativePath(rootFile))).iterator();
    }
