    ...
    public static class RestoreFileAttributesFilter extends AbstractFilter<RestoreFileAttributesConfig> {
        ...
        public void filter(ObjectContext objectContext) {
            ...
            if (!link) { // cannot set times for symlinks in Java
                ...
                try {
                    log.debug("setting times of {} to mtime:{} atime:{} crtime:{}", file.getPath(), mtime, atime, crtime);
            ...
            try {
                ...
                if (permissions != null && !link) {
                    log.debug("setting permissions of {} to {}", file.getPath(), metadata.getUserMetadataValue(META_POSIX_MODE));
                ...
                if (uid != null) {
                    log.debug("setting ownership of {} to {}.{}", file.getPath(), uid, gid);
                    ...
                    if (ownerName != null) {
                        log.debug("setting ownership of {} to {}", file.getPath(), ownerName);
                        attributeView.setOwner(lookupService.lookupPrincipalByName(ownerName));
                    }
                    if (groupOwnerName != null) {
                        log.debug("setting group-ownership of {} to {}", file.getPath(), groupOwnerName);
