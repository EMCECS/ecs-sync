...
public class AtmosTarget extends SyncTarget {
    ...
    private static final Logger log = LoggerFactory.getLogger(AtmosTarget.class);
    ...
    public Options getCustomOptions() {
        ...
        opts.addOption(Option.builder().longOpt(DEST_NO_UPDATE_OPTION).desc(DEST_NO_UPDATE_DESC).build());
        opts.addOption(Option.builder().longOpt(DEST_CHECKSUM_OPT).desc(DEST_CHECKSUM_DESC)
                .hasArg().argName(DEST_CHECKSUM_ARG_NAME).build());
        opts.addOption(Option.builder().longOpt(RETENTION_DELAY_WINDOW_OPTION).desc(RETENTION_DELAY_WINDOW_DESC)
                .hasArg().argName(RETENTION_DELAY_WINDOW_ARG_NAME).build());
        opts.addOption(Option.builder().longOpt(REPLACE_META_OPTION).desc(REPLACE_META_DESC).build());
        return opts;
    }
    ...
    public void configure(SyncSource source, Iterator<SyncFilter> filters, SyncTarget target) {
        ...
        if (noUpdate)
            log.info("Overwrite/update target objects disabled");

        if (includeRetentionExpiration)
            log.info("Retention start delay window set to " + retentionDelayWindow);
    }
    ...
    public void filter(final SyncObject obj) {
        ...
        if ("/".equals(destNamespace + obj.getRelativePath())) {
            log.debug("Target namespace is root");
            return;
        }
            ...
            if (targetSystemMeta == null) { // CREATE
                ...
                if (obj.isDirectory()) { // CREATE DIRECTORY
                    ...
                    if (!(targetId instanceof ObjectPath)) {
                        log.debug("Source {} is a directory, but target is in objectspace, ignoring",
                    ...
                    if (checksum != null) {
                        ...
                        try (InputStream in = obj.getInputStream()) {
                            ...
                            request.contentLength(obj.getMetadata().getContentLength()).contentType(obj.getMetadata().getContentType());
                ...
                if (noUpdate) {
                    log.debug("Skipping {}, updates disabled.", obj.getTargetIdentifier());
                } else {
                    ...
                    if (obj.isDirectory()) { // UPDATE DIRECTORY
                        ...
                        if (dataChanged(obj, targetSystemMeta) || force) {
                            ...
                            log.debug("No changes from source {} to dest {}",
    ...
    public SyncObject reverseFilter(SyncObject obj) {
        ObjectIdentifier targetId = getTargetId(obj);
        if (targetId != null) obj.setTargetIdentifier(targetId.toString());
