...
public class MetadataFilterTest {
    ...
    public void testChangeKeys() {
        ...
        Assert.assertEquals(v2, target.objectMap.get(key1).getMetadata().getUserMetadataValue(k2));
        Assert.assertEquals(v3, target.objectMap.get(key1).getMetadata().getUserMetadataValue(k3));
        Assert.assertEquals(2, target.objectMap.get(key1).getMetadata().getUserMetadata().size());
        Assert.assertNull(target.objectMap.get(key1).getMetadata().getUserMetadataValue(k1));

        Assert.assertEquals(v1, target.objectMap.get(key2).getMetadata().getUserMetadataValue(newKey));
        Assert.assertEquals(v2, target.objectMap.get(key2).getMetadata().getUserMetadataValue(k2));
        Assert.assertEquals(2, target.objectMap.get(key2).getMetadata().getUserMetadata().size());
        Assert.assertNull(target.objectMap.get(key2).getMetadata().getUserMetadataValue(k1));
    }
    ...
    private MetadataFilter buildFilter(MetadataConfig config, Target target) {
        ...
        filter.configure(storage, Collections.singletonList((SyncFilter) filter).iterator(), storage);
