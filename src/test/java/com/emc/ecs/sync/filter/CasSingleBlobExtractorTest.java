...
public class CasSingleBlobExtractorTest {
    ...
    public void setup() throws Exception {
        ...
        try {
            ...
            Assume.assumeNotNull(casConnection);
    ...
    private void verify(TestStorage target) {
        for (String key : TARGET_PATH_VALUE_PASS) {
            ...
            Assert.assertEquals("target path does not match", key, testSyncObject.getRelativePath());
            for (ObjectMetadata.UserMetadata uMeta : testSyncObject.getMetadata().getUserMetadata().values()) {
                Assert.assertEquals("attribute name found was not expected",
                        true, Arrays.asList(ATTR_NAMES).contains(uMeta.getKey()));
                Assert.assertEquals("attribute value was not expected",
                        true, Arrays.asList(ATTR_VALUES).contains(uMeta.getValue()));
            }
            if (testSyncObject.getData().length > 0) // might have come from clip with no blob
                Assert.assertEquals("blob content did not match",
                        true, Arrays.equals(blobContent, testSyncObject.getData()));
        }
    }
