...
public class AzureBlobStorageTest {
    ...
    private String bucketName = "ecs-sync-azure-s3-test-ut2";
...
//        targetConfig.setIncludeVersions(true);
    ...
    public void testListAllObjectInAzureBlob() throws URISyntaxException, StorageException {
        ...
        log.info("objectsummaries: {}", objectSummaries.size());

        CloudBlobContainer container = blobClient.getContainerReference(containerName);
        List<CloudBlob> cloudBlobList = new ArrayList<>();
        for (ListBlobItem blob : container.listBlobs()) {
            listAllBlobs(cloudBlobList, blob, null);
        }
        Assert.assertEquals("blob number of sync mismatch with Azure blob", objectSummaries.size(), cloudBlobList.size());
    }

    private void listAllBlobs(List<CloudBlob> blobs, ListBlobItem blobItem, String perfix) {
        if (blobItem instanceof CloudBlobDirectory) {
            try {
                for (ListBlobItem blob : ((CloudBlobDirectory) blobItem).listBlobs(perfix)) {
                    listAllBlobs(blobs, blob, perfix);
                }
            } catch (URISyntaxException | StorageException e) {
                throw new RuntimeException("error for list all Azure blobs");
            }
        } else if (blobItem instanceof CloudBlob) {
            CloudBlob targetBlob = (CloudBlob) blobItem;
            blobs.add(targetBlob);
        } else {
            throw new RuntimeException("wrong blob type: " + blobItem.getUri());
        }
    }
    ...
    public void testLoadObjectWithoutSnapshots() throws URISyntaxException, StorageException, IOException {
        ...
        Assert.assertEquals("sync data missmatch with source", sourceMd5Hex, syncMd5Bytes);
    }
