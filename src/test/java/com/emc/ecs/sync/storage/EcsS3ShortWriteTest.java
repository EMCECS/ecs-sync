...
public class EcsS3ShortWriteTest {
    ...
    public void testMockServer() throws IOException {
        ...
        try {
            ...
            Assert.assertEquals(MOCK_OBJ_SIZE, response.getLength());
            // and chunked-encoding *was* used
            Assert.assertEquals("chunked", response.getHeaders().getFirst("Transfer-encoding"));
            // but only half the data was sent
            Assert.assertEquals(MOCK_TRUNC_SIZE, response.getEntity(byte[].class).length);
            ...
            Assert.assertNull(response.getHeaders().getFirst("Transfer-encoding"));
            try {
                ...
                Assert.fail("Apache httpclient should have thrown exception in non-chunked read");
            } catch (ClientHandlerException e) {
                Assert.assertTrue(e.getCause() instanceof ConnectionClosedException);
                Assert.assertTrue(e.getCause().getMessage().contains("Premature end of Content-Length"));
            }
            ...
            Assert.assertEquals("chunked", chunkedObject.getKey());
            Assert.assertNotNull(chunkedObject.getLastModified());
            Assert.assertEquals(new Long(MOCK_OBJ_SIZE), chunkedObject.getSize());
            Assert.assertNotNull(chunkedObject.getETag());
            Assert.assertEquals("joe", chunkedObject.getOwner().getId());
            S3Object nonChunkedObject = result.getObjects().get(1);
            Assert.assertEquals("non-chunked", nonChunkedObject.getKey());
            Assert.assertNotNull(nonChunkedObject.getLastModified());
            Assert.assertEquals(new Long(MOCK_OBJ_SIZE), nonChunkedObject.getSize());
            Assert.assertNotNull(nonChunkedObject.getETag());
            Assert.assertEquals("joe", nonChunkedObject.getOwner().getId());
        } finally {
            mockServer.stop(10);
        }
    }
    ...
    public void testTruncatedReadErrors() throws Exception {
        ...
        try {
            ...
            Assert.assertEquals(2, sync.getStats().getObjectsFailed());
        } finally {
            mockServer.stop(10);
        }
    }
    ...
    public void testTruncatedWriteApache() throws IOException {
        ...
        try {
            ...
            Assert.assertEquals(0, s3.listObjects(BUCKET_NAME).getObjects().size());
        } finally {
            mockServer.stop(10);
        }
    }
