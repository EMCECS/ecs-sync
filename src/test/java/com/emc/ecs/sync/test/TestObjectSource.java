...
public class TestObjectSource extends SyncSource<TestSyncObject> {
    private static final Logger log = LoggerFactory.getLogger(TestObjectSource.class);
    ...
    protected void estimate(List<TestSyncObject> objects, SyncEstimate estimate) {
        for (TestSyncObject object : objects) {
            estimate.incTotalObjectCount(1);
            if (!object.isDirectory()) estimate.incTotalByteCount(object.getData().length);
            else estimate(object.getChildren(), estimate);
        }
    }
    ...
    public void configure(SyncSource source, Iterator<SyncFilter> filters, SyncTarget target) {
        if (objects == null) objects = generateRandomObjects(null, objectCount, maxSize, 1);
    }
    ...
    public void setObjects(List<TestSyncObject> objects) {
        this.objects = objects;
    }
