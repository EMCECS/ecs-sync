...
public class LazyTest {
    ...
    public void testLazyValue() {
        ...
        Assert.assertFalse(initialized.get());
    ...
    public void testLazyObject() throws Exception {
        ...
        Assert.assertFalse(aclInitialized.get());
        Assert.assertFalse(streamInitialized.get());

        InputStream stream = object.getDataStream();
        Assert.assertTrue(streamInitialized.get());

        // just for safety
        Assert.assertEquals(-1, stream.read(new byte[1]));

        object.getAcl();
        Assert.assertTrue(aclInitialized.get());
    }
}
