...
public class PreserveFiltersTest {
    ...
    public void testPosixAclPreservation() throws Exception {
        ...
        for (File sourceFile : sourceDir.listFiles()) {
            ...
            if (isRoot) {
                Assertions.assertEquals(uid, Files.getAttribute(targetFile.toPath(), "unix:uid"));
                Assertions.assertEquals(gid, Files.getAttribute(targetFile.toPath(), "unix:gid"));
            }
            Assertions.assertEquals(permissions, Files.getPosixFilePermissions(targetFile.toPath()));
        }
    }
    ...
    public void testPreserveFileAttributes() throws Exception {
        ...
        for (SyncObject object : testStorage.getRootObjects()) {
            ...
            if (mtime == null) Assertions.assertNull(mtimeSecs);
            else Assertions.assertEquals(mtime.toMillis() / 10, (long) (Double.parseDouble(mtimeSecs) * 100D));
            String atimeSecs = object.getMetadata().getUserMetadataValue(PreserveFilters.META_ATIME);
            if (atime == null) Assertions.assertNull(atimeSecs);
            else // atime is affected by reading the file times
                Assertions.assertTrue(Math.abs(atime.toMillis() - (long) (Double.parseDouble(atimeSecs) * 1000D)) < 1000);
            String crtimeSecs = object.getMetadata().getUserMetadataValue(PreserveFilters.META_CRTIME);
            if (crtime == null) Assertions.assertNull(crtimeSecs);
            else Assertions.assertEquals(crtime.toMillis() / 10, (long) (Double.parseDouble(crtimeSecs) * 100D));
            ...
            if (ctime == null) Assertions.assertNull(ctimeSecs);
            else Assertions.assertEquals(ctime.toMillis() / 10, (long) (Double.parseDouble(ctimeSecs) * 100D));

            // check permissions
            if (isRoot) {
                Assertions.assertEquals(uid.toString(), object.getMetadata().getUserMetadataValue(PreserveFilters.META_OWNER));
                Assertions.assertEquals(gid.toString(), object.getMetadata().getUserMetadataValue(PreserveFilters.META_GROUP));
            }
            Assertions.assertEquals(mode, object.getMetadata().getUserMetadataValue(PreserveFilters.META_PERMISSIONS));
        }
    }
    ...
    public void testRestoreFileAttributes() throws Exception {
        ...
        for (File sourceFile : sourceDir.listFiles()) {
            ...
            // check permissions
            if (isRoot) Assertions.assertEquals(uid, Files.getAttribute(targetFile.toPath(), "unix:uid"));
            if (isRoot) Assertions.assertEquals(gid, Files.getAttribute(targetFile.toPath(), "unix:gid"));
            Assertions.assertEquals(permissions, Files.getPosixFilePermissions(targetFile.toPath()));
        }
    }
    ...
    public void testRestoreSymLink() throws Exception {
        ...
        try (OutputStream out = new FileOutputStream(new File(sourceDir, file))) {
            SyncUtil.copy(new RandomInputStream(1024), out, 1024);
        }
        ...
        if (isRoot) {
            Assertions.assertEquals(gid, Files.getAttribute(Paths.get(targetDir.getPath(), file), "unix:gid", LinkOption.NOFOLLOW_LINKS));
            Assertions.assertEquals(gid, Files.getAttribute(Paths.get(targetDir.getPath(), link), "unix:gid", LinkOption.NOFOLLOW_LINKS));
        }
        ...
        Assertions.assertEquals(fileMtime.getTime() / 10, tFileMtime.getTime() / 10);
//        Assertions.assertEquals(linkMtime, tLinkMtime); // TODO: figure out a way to set mtime on a link in Java
    }
    ...
    private void writeTestFiles(File dir, int count, int size, int uid, int gid, Set<PosixFilePermission> permissions)
        ...
        for (int i = 0; i < count; i++) {
            ...
            SyncUtil.copy(new RandomInputStream(size), new FileOutputStream(file), size);
            // set uid/gid/mode
