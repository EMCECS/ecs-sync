...
public final class CliHelper {
    ...
    public static CliConfig parseCliConfig(String[] args) {
        try {
            CliConfig cliConfig;
            if (args.length == 0) {
            ...
            if (cliConfig.isHelp() || cliConfig.isVersion()) {
                ...
                if (!cliConfig.isRestOnly() && cliConfig.getXmlConfig() == null
                    ...
                    throw new ConfigurationException("Source and target options should be specified first");
                }
                return cliConfig;
            }
    ...
    public static SyncConfig parseSyncConfig(CliConfig cliConfig, String[] args) {
        // main CLI options
        ...
        if (RoleType.Target == sourceWrapper.getRole())
            throw new ConfigurationException(sourceWrapper.getLabel() + " cannot be used as a source");
        ...
        if (RoleType.Source == targetWrapper.getRole())
            throw new ConfigurationException(targetWrapper.getLabel() + " cannot be used as a target");
